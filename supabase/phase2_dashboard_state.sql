-- Rhythm Agent Phase 2 dashboard-state extension.
-- Keeps the Phase 1 action semantics and adds the selected movement plan.
...
begin
  ...
  if v_action not in ('get_state', 'complete', 'start', 'reset_today') then
    raise exception 'Unsupported Rhythm action: %', v_action using errcode = '22023';
  end if;
  ...
  if v_plan_id is null then
    return jsonb_build_object(
      'ok', true, 'action', v_action, 'date', p_date::text,
      'needsSetup', true, 'energyMode', 'normal', 'movement', null,
      'now', null, 'next', null, 'later', '[]'::jsonb,
      'completed', '[]'::jsonb, 'chunks', '[]'::jsonb
    );
  end if;

  if v_action <> 'get_state' then
    perform c.id from public.chunks c
    where c.daily_plan_id = v_plan_id and c.owner_id = v_uid
    order by c.position for update;
  end if;
  ...
  if v_action = 'reset_today' then
    ...
    set status = case when ordered.sequence = 1 then 'current' when ordered.sequence = 2 then 'next' else 'later' end,
        started_at = case when ordered.sequence = 1 then v_now else null end,
        completed_at = null,
        updated_at = v_now
    from ordered where c.id = ordered.id;

    update public.daily_plans set status = 'active', updated_at = v_now
    where id = v_plan_id and owner_id = v_uid;
  elsif v_action = 'complete' then
    select c.id into v_current_id
    from public.chunks c
    where c.daily_plan_id = v_plan_id and c.owner_id = v_uid and c.status = 'current'
    limit 1;
    ...
    select c.id, c.position into v_next_id, v_next_position
    from public.chunks c
    where c.daily_plan_id = v_plan_id and c.owner_id = v_uid and c.status = 'next'
    limit 1;

    if v_next_id is not null then
      select c.id into v_following_id
      from public.chunks c
      where c.daily_plan_id = v_plan_id and c.owner_id = v_uid
        and c.status = 'later' and c.position > v_next_position
      order by c.position limit 1;
    end if;

    update public.chunks
    set status = 'completed', completed_at = v_now, updated_at = v_now
    where id = v_current_id and owner_id = v_uid;

    if v_next_id is not null then
      update public.chunks
      set status = 'current', started_at = coalesce(started_at, v_now), updated_at = v_now
      where id = v_next_id and owner_id = v_uid;
    end if;

    if v_following_id is not null then
      update public.chunks set status = 'next', updated_at = v_now
      where id = v_following_id and owner_id = v_uid;
    end if;
  elsif v_action = 'start' then
    select c.position into v_selected_position
    from public.chunks c
    where c.id = p_chunk_id and c.daily_plan_id = v_plan_id and c.owner_id = v_uid
    ...
    select c.id into v_following_id
    from public.chunks c
    where c.daily_plan_id = v_plan_id and c.owner_id = v_uid and c.id <> p_chunk_id
      and c.status not in ('completed', 'skipped') and c.position > v_selected_position
    order by c.position limit 1;

    update public.chunks set status = 'later', updated_at = v_now
    where daily_plan_id = v_plan_id and owner_id = v_uid and status in ('current', 'next', 'later');

    update public.chunks
    set status = 'current', started_at = coalesce(started_at, v_now), updated_at = v_now
    where id = p_chunk_id and owner_id = v_uid;
  ...
  with visible as (
    select c.position, c.status,
      jsonb_build_object(
        'id', c.id, 'daily_plan_id', c.daily_plan_id, 'template_key', c.template_key,
        'title', c.title, 'position', c.position, 'status', c.status,
        'transition_cue', c.transition_cue, 'ease_level', c.ease_level,
  ...
  select jsonb_build_object(
    ...
    'energyMode', coalesce(dp.energy_mode, 'normal'),
